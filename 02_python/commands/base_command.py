...
BaseCommand에서의 구현에서는 분기가 필요하기 때문이다. 이는 매 command마다 execute가 다르게 구현되어야 하기 때문에 여러 private 변수들을 참조하는 것이 복잡하기 때문이다.
...
@classmethod는 인스턴스화 없이도 호출 가능한 메소드이기 때문이고
BaseCommand 클래스는 인스턴스화 없이도 update_current_path()를 호출 가능해야 하기 때문에 self 대신 cls 인자를 받는 형태로 구현되었다.
"""
class BaseCommand:
    """
